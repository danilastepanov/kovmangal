...
window.onload = function () {
    ...
    for (let i = 0; i < radio.length; i++) {
        radio[i].onchange = function () {
            let messenger = document.querySelectorAll(".form__label-socials");
            messenger.forEach(function (e) {
                if (e.classList.contains("form__label-socials-popup")) {
                    changeMessenger(e, i);
                }
                if (e.classList.contains("form__label-socials-main")) {
                    changeMessenger(e, i);
                }
            });
        };
    ...
    function changeMessenger(e, i) {
        e.innerHTML = radio[i].value;
        for (let key of arr.keys()) {
            if (radio[i].value == key) {
                e.style.color = arr.get(key);
            }
        }
    }
    ...
    if (header) {
        ...
        window.addEventListener("scroll", function () {
            t() > e && !header.classList.contains("_hide") && t() > i
                ? header.classList.add("_hide")
                : t() < e && header.classList.contains("_hide") && header.classList.remove("_hide"),
                (e = t());
        });
    }
};
